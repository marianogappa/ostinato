...
class ThreatenedDefendedTest extends FunSpec with ShouldMatchers {
  describe("ChessGame threatened/defended pieces") {
    it("should find no actions for King if it's threatened in every direction, other than draw and resign") {
      ...
      game.board.actions shouldBe Set(DrawAction(WhiteChessPlayer), WinAction(BlackChessPlayer))
    }
