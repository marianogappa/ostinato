...
class ChessBoardTest extends FunSpec with Matchers {
  describe("ChessBoard") {
    it(
      "should increase the half move clock count after a non capture nor pawn advance") {
      ...
      val board2 = board
        .doAction(game.whitePlayer.knights(board).head.actions(board).head)
        .get
    ...
    it("should zero the half move clock count after a Pawn movement") {
      ...
      val board2 = board
        .doAction(game.whitePlayer.pawns(board).head.actions(board).head)
        .get
    ...
    it("should change turn after an action when on white's turn") {
      val board = ChessGame.defaultGame.board
      board
        .doAction(board.nonFinalActions.head)
        .get
        .turn shouldBe BlackChessPlayer
    }
    ...
    it("should change turn after a action when on blacks's turn") {
      ...
      game.board
        .doAction(game.board.nonFinalActions.head)
        .get
        .turn shouldBe WhiteChessPlayer
    }

    it("should play a lot and not stack overflow") {
      def doActionsUntilLockedOrNActions(board: ChessBoard =
                                           ChessGame.defaultGame.board,
                                         n: Int = 200): Unit = {
        val actions = board.nonFinalActions
        if (n > 0 && actions.nonEmpty)
          doActionsUntilLockedOrNActions(board.doAction(actions.head).get,
                                         n - 1)
        else
          ()
      }
