...
case class CoordinateNotationRules(
...
case class CoordinateNotationActionSerialiser(r: CoordinateNotationRules) extends ActionSerialiser {
  ...
  protected def capture(a: CaptureAction) = fromPos(a) * captureDash * toPos(a) * checkAndCheckmate(a)
  protected def enPassantCapture(a: EnPassantCaptureAction) = fromPos(a) * captureDash * toPos(a) * checkAndCheckmate(a)
  ...
  private def captureDash = if (r.useDashDelimiter) if (r.distinguishCaptures) Set("x") else Set("-") else Set("")
