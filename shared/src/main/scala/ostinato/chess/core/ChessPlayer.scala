...
abstract class ChessPlayer(name: String)
    extends Player[ChessBoard,
                   ChessAction,
                   ChessPiece,
                   ChessPlayer,
                   ChessOptimisations](name) {
  def kingPiece(board: ChessBoard): Option[ChessPiece] =
    pieces(board).find(_.isKing)
  ...
  override def actions(
    ...
    (board.hasInsufficientMaterial, super.actions(board)(o), kingPiece(board)) match {
      ...
      case (_, a, Some(k)) if a.isEmpty ⇒
        Set(if (k.isThreatened(board)) LoseAction(this) else DrawAction(this))
  ...
  override def actionStream(board: ChessBoard)(
    ...
    (board.hasInsufficientMaterial,
     super.actionStream(board)(o),
     kingPiece(board)) match {
      case (true, _, _) ⇒ Stream(DrawAction(this))
      case (_, a, Some(k)) if a.isEmpty ⇒
        Stream(
          if (k.isThreatened(board)) LoseAction(this) else DrawAction(this))
      case (_, a, _) ⇒ a
    }
  }

  def nonFinalActions(board: ChessBoard)(implicit opts: ChessOptimisations =
  ...
  def rooks(board: ChessBoard): Set[♜] = pieces(board) flatMap {
    case p: ♜ ⇒ Set(p); case _ ⇒ Set.empty[♜]
  }

  def knights(board: ChessBoard): Set[♞] = pieces(board) flatMap {
    case p: ♞ ⇒ Set(p); case _ ⇒ Set.empty[♞]
  }

  def bishops(board: ChessBoard): Set[♝] = pieces(board) flatMap {
    case p: ♝ ⇒ Set(p); case _ ⇒ Set.empty[♝]
  }

  def queens(board: ChessBoard): Set[♛] = pieces(board) flatMap {
    case p: ♛ ⇒ Set(p); case _ ⇒ Set.empty[♛]
  }

  def kings(board: ChessBoard): Set[♚] = pieces(board) flatMap {
    case p: ♚ ⇒ Set(p); case _ ⇒ Set.empty[♚]
  }

  def pawns(board: ChessBoard): Set[♟] = pieces(board) flatMap {
    case p: ♟ ⇒ Set(p); case _ ⇒ Set.empty[♟]
  }
}
