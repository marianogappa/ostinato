...
object ChessGame {
  ...
  def fromFen(fenString: String): Try[ChessGame] =
    if (Fen.isValidFen(fenString)) {
      ...
      val (gridS,
           ...
           enPassantS,
           halfMoveCountS,
           fullMoveNumberS) = (s(0), s(1), s(2), s(3), s(4), s(5))
      val castlingAvailable =
        Fen.calculateCastlingAvailable(castlingAvailableS)
      val turn = Fen.calculateTurn(turnS)

      val grid = ChessGrid.fromGridString(
        gridS.map(Fen.shortFenTransformation(_)).mkString)
      val halfMoveCount = Fen.calculateNumber(halfMoveCountS)
      val fullMoveNumber = Fen.calculateNumber(fullMoveNumberS)
      val enPassantPawn = turn flatMap (t ⇒
        Fen.calculateEnPassantPawn(enPassantS, t))
      ...
      (grid, turn, enPassantPawn, halfMoveCount, fullMoveNumber) match {
        ...
        case (_grid,
              Some(_turn),
              _epp,
              Success(_halfMoveCount),
              Success(_fullMoveNumber)) ⇒
          Success(
            ChessGame(
              ChessBoard(_grid,
                         _turn,
  ...
  def fromShortFen(shortFenString: String): Try[ChessGame] =
    if (Fen.isValidShortFen(shortFenString)) {
      val grid = ChessGrid.fromGridString(
        shortFenString.map(Fen.shortFenTransformation(_)).mkString)
...
case class ChessGame(override val board: ChessBoard,
                     override val rules: ChessOptimisations)
    extends Game[ChessBoard,
                 ChessAction,
                 ChessPiece,
                 ChessPlayer,
                 ChessOptimisations](board, chessPlayers, rules) {
  ...
  def isGameOver(
    ...
    : Boolean = isDraw || lossFor.nonEmpty

  def lossFor(implicit opts: ChessOptimisations = ChessOptimisations.default)
    : Option[ChessPlayer] = players find (board.isLossFor(_) == true)

  def isDraw(implicit opts: ChessOptimisations = ChessOptimisations.default)
    : Boolean = board.isDraw
...
case object InvalidChessGridSizeException
    extends RuntimeException
    with NoStackTrace

case object InvalidFullMoveNumberException
    extends RuntimeException
    with NoStackTrace

case object InvalidHalfMoveCountException
    extends RuntimeException
    with NoStackTrace

case object MoreThanOneEnPassantPawnException
    extends RuntimeException
    with NoStackTrace

case object FenStringRegexMismatchException
    extends RuntimeException
    with NoStackTrace

case object OstinatoStringRegexMismatchException
    extends RuntimeException
    with NoStackTrace
