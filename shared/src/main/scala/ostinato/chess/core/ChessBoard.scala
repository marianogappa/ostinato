...
case class ChessBoard(
    ...
    halfMoveClock: Int = 0,
    history: List[GameStep] = List()) extends Board[ChessBoard, ChessAction, ChessPiece, ChessPlayer, ChessRules](grid) {
  ...
  def doAction(a: ChessAction)(implicit rules: ChessRules = ChessRules.default) = {
    ...
    lazy val applyAction = {
      val newBoard = ChessBoard(
        ...
        fullMoveNumber = if (turn == BlackChessPlayer) fullMoveNumber + 1 else fullMoveNumber,
        halfMoveClock = calculateHalfMoveClock
      )

      Some(newBoard.copy(history = GameStep(a, newBoard) :: history))
    }
  ...
  def movementsOfDelta(from: XY, delta: XY)(implicit rules: ChessRules = ChessRules.default): Set[ChessAction] = {
    ...
    lazy val validateAction: Set[ChessActionFactory] = (fromPiece, toPiece, enPassantPawn) match {
      case (Some(Some(p: ♟)), Some(None), Some(epp: EnPassantPawn)) if delta.x != 0
  ...
  lazy val toShortFen: String = grid.map {
    case Some(c) ⇒ c.toFen
    case _       ⇒ ' '
  ...
  lazy val isInStalemate = turn.actions(this) == Set(DrawAction(turn, isCheck = false, isCheckmate = false))
