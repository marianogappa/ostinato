...
case class ChessBasicAi(player: ChessPlayer,
                        depth: Int = 1,
                        debug: Boolean = false)
    extends Ai[ChessBoard,
               ChessAction,
               ChessPiece,
               ChessPlayer,
               ChessOptimisations,
  ...
  override def nextAction(
      game: ChessGame)(implicit opts: ChessOptimisations =
                         ChessOptimisations.default): Option[ChessAction] = {
    val optsForAi = opts.copy(extraValidationOnActionApply = false,
    ...
    if (debug) {
      println(s"Chosen action: $chosenAction")
    }

    Some(chosenAction)
  }

  def evaluateAllActions(
      actions: Stream[ChessAction],
      game: ChessGame,
      optsForAi: ChessOptimisations): Seq[(ChessAction, Long)] =
    actions map (action ⇒
      (action,
       alphabeta(game.board.doAction(action)(optsForAi).get, action)(
         optsForAi)))
  ...
  def alphabeta(
      ...
      action: ChessAction,
      depth: Int = depth,
      alpha: Long = -Long.MaxValue,
      beta: Long = Long.MaxValue)(implicit opts: ChessOptimisations): Long = {
    var a = alpha
    var b = beta
    ...
    if (depth == 0) {
      ...
      if (player != action.turn) {
        ...
        board.actionStream.foreach { newAction ⇒
          v = math.max(v,
                       alphabeta(board.doAction(newAction).get,
                                 newAction,
                                 depth - 1,
                                 a,
                                 b))
  ...
  private def evaluate(board: ChessBoard, action: ChessAction)(
      implicit opts: ChessOptimisations = ChessOptimisations.default): Long = {
    action match {
      ...
      case _ ⇒
        if (action.isCheckmate) {
          ...
          val undefendedOwnPieces =
            action.turn.pieces(board).filter(!_.isDefended(board))
          val undefendedThreatenedOwnPieces =
            undefendedOwnPieces.filter(_.isThreatened(board))

          val enemyMaterialValue =
            action.turn.enemy.pieces(board).toList.map(materialValue).sum
          val playerMaterialValueWithoutLikelyLostValue =
            action.turn.pieces(board).toList.map(materialValue).sum
          val enemyHasMaterialValueAdvantage = enemyMaterialValue > playerMaterialValueWithoutLikelyLostValue

          val likelyLostValue = undefendedThreatenedOwnPieces.toList
            .sortBy(materialValue)
            .lastOption
            .map(materialValue)
            .getOrElse {
              if (enemyHasMaterialValueAdvantage) {
                val threatenedOwnPieces = action.turn.pieces(board).flatMap {
                  ownPiece =>
                    ownPiece.threatenedBy(board) match {
                      case Some(enemyPiece)
                          if materialValue(ownPiece) >= materialValue(
                            enemyPiece) =>
                        Set(materialValue(ownPiece))
          ...
          val totalMaterialValue = playerMaterialValue * sign(action.turn) + enemyMaterialValue * sign(
            action.turn.enemy)
          ...
          val undefendedThreatenedOwnPieceValue =
            undefendedThreatenedOwnPieces.toList.map(materialValue).sum

          // PRIORITY #5: UNDEFENDED OWN PIECE VALUE
          val undefendedOwnPieceValue =
            undefendedOwnPieces.toList.map(materialValue).sum

          // PRIORITY #6: UNDEFENDED THREATENED ENEMY PIECE VALUE
          val undefendedEnemyPieces =
            action.turn.enemy.pieces(board).filter(!_.isDefended(board))
          val undefendedThreatenedEnemyPieceValue = undefendedEnemyPieces
            .filter(_.isThreatened(board))
            .toList
            .map(materialValue)
            .sum

          // PRIORITY #7: UNDEFENDED ENEMY PIECE VALUE
          val undefendedEnemyPieceValue =
            undefendedEnemyPieces.toList.map(materialValue).sum
          ...
          val totalDistanceToPromotion =
            action.turn.pawns(board).map(_.distanceToPromotion).sum
          val totalDistanceToPromotionMultiplier =
            if (gameStage == "early") 1
            else if (gameStage == "middle") 10
            else 100
          val hasMiddle = action.turn
            .pieces(board)
            .count(isInMiddle) > action.turn.enemy
            .pieces(board)
            .count(isInMiddle)
          val hasMiddleMultiplier = if (gameStage == "late") 0 else 1
          val enemyHasMiddle = action.turn.enemy
            .pieces(board)
            .count(isInMiddle) > action.turn.pieces(board).count(isInMiddle)

          val couldCastleButMovesKing = {
            val canCastle =
              board.castlingAvailable((action.turn, CastlingSide.Kingside)) || board
                .castlingAvailable((action.turn, CastlingSide.Queenside))
          ...
          val idleBishopKnightPenalty =
            if (gameStage == "early")
              (action.turn.knights(board) ++ action.turn.bishops(board))
                .count(_.pos.y == action.turn.initialY)
            else
              0
          ...
          val nonIdleRookPenalty =
            if (gameStage == "early")
              action.turn
                .rooks(board)
                .count(r =>
                  r.pos.y != action.turn.initialY || (r.pos.x != 0 && r.pos.x != 7))
            else
              0
          ...
          val value: Long =
            int(castles) * 100 * sign(action.turn) +
              ...
              int(enemyHasMiddle) * -100 * hasMiddleMultiplier * sign(
                action.turn) +
              totalDistanceToPromotion * totalDistanceToPromotionMultiplier * sign(
                action.turn) +
